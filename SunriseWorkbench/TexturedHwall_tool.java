...
public class TexturedHwall_tool extends RoboticsAPIApplication {
	...
	public void initialize() {
		...
		getLogger().info("move robot with initial vel: " + initial_velocity );
		robot.move(new PTP(new JointPosition(0, 0.523599, 0, -1.5708, 0, -0.523599, 1.5708)).setJointVelocityRel(initial_velocity));
			...
			while (true) {
		       ...
		       if (subscriber.currentCommandType != null) {
		    	   ...
		    	   case CARTESIAN_POSE: {
		    		   ...
		    		   force_vec = robot.getExternalForceTorque(tool_lin.getDefaultMotionFrame()).getForce();
			           ...
			           ///////contact state : tool coordinate
			        	   ...
			        	   rel_vel = (-1)*commandPosition.getPose().getOrientation().getW()/5000000000L; //Opposite direction

			        	   if(!contact) //contact == false
			        	   {
			        		   degree = commandPosition.getPose().getOrientation().getY();
			               ...
			               while (true)
			               {
			                   ...
			                   //adjust degree
			                   commandPosition = subscriber.getCartesianPose();

			                   setori = degree - commandPosition.getPose().getOrientation().getY();
			                   setx += (curTime)*rel_vel*Math.cos(setori+degree);
			                   sety += (curTime)*rel_vel*Math.sin(setori+degree);

			                   destFrame.setX(setx);
			                   destFrame.setY(sety);
			                   destFrame.setAlphaRad(setori);
