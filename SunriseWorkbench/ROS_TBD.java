...
public class ROS_TBD extends RoboticsAPIApplication {
 private Controller cabinet;
 private LBR robot;
 private Tool tool;
 private SmartServo motion;
 private Lock configureSmartServoLock = new ReentrantLock();
 private double initial_velocity = 0.07;
 private ObjectFrame tcp_tool;
 private CartesianImpedanceControlMode redundCIC = new CartesianImpedanceControlMode(); //TODO: need to be changed to joint impedance control mode [yaesol]
 
 private JointPosition j_start, j_end;
 ...
 public void initialize() {
   ...
   getLogger().info("Move to the initial pose with initial vel: " + initial_velocity);
   //robot.move(new PTP(home1).setJointVelocityRel(1));   
   robot.move(new PTP(new JointPosition(0, 0.4433136, 0, -1.5708, 0, 1.1067133, -0.61679936 )).setJointVelocityRel(initial_velocity));
   ...
   //redundCIC.parametrize(CartDOF.B, CartDOF.C).setStiffness(3).setDamping(0.1);
   //redundCIC.parametrize(CartDOF.A).setStiffness(3).setDamping(0.1);
 ...
 public void run() {	 
	 ...
	 // save the start and end point manually
	 ...
	 while (end_flag) {
		 ...
		 int ret = getApplicationUI().displayModalDialog(ApplicationDialogType.QUESTION, "Save this position?", "Save start point", "Save end point", "DONE");
		 ...
		 case 0:
			 ...
			 getLogger().info("J: [0]" + robot.getCurrentJointPosition().get(0) + "[1]" + robot.getCurrentJointPosition().get(1)+ "[2]" + robot.getCurrentJointPosition().get(2) );
			 break;			 
		 case 1:
			 pointQ = new Frame();
			 pointQ = robot.getCurrentCartesianPosition(tcp_tool);
			 j_end = robot.getCurrentJointPosition();
			 
			 try {
				publisher.publishCurrentState(robot, motion);
				//publisher.setPublishJointStates(configuration.getPublishJointStates());
				getLogger().info("publish the end state.");
			} catch (InterruptedException e) {
