		...
		if (isRobotConnected && use_cartesian_command)
		{
			...
			else //x or y is not a zero
			{
				...
				if(movemode) //if it was move mode, change the mode: move -> touch
				{
					...
					ROS_INFO("0. pos rel wall: %f", pos_rel_wall);

					pos_rel_wall = 0;
					do
					{
						ROS_INFO("1. pos rel wall: %f", pos_rel_wall);
						ROS_INFO("Waiting for messages from unity");
						str_len = recvfrom(serv_sock, VAL, 12, 0, (struct sockaddr *)&clnt_adr, &clnt_adr_sz);
						memcpy(&ori_f, &VAL, sizeof(ori_f));
						memcpy(&pos_rel_wall, &VAL[4], sizeof(pos_rel_wall));
						memcpy(&control_mode, &VAL[8], sizeof(control_mode));
						ROS_INFO("recieve messages from unity");
						ROS_INFO("2. pos rel wall: %f", pos_rel_wall);
					} while(pos_rel_wall == 0); //if tag is zero, it means it do not have ori_f.

					//now we have ori_f.
					ROS_INFO("ori_f, pos_rel_wall, control mode: %f, %f, %f", ori_f, pos_rel_wall, control_mode); //ori_f: 135, 150, 165, 180, 195, 210, 225

					//ROS_INFO("head x: %f, robot x: %f",forward_headx, forward_headx/1000 - 0.68); //forward
					//robot_x = forward_headx/100 - 0.64;
					robot_x = 0.53;
				...
				else // just touch mode
				{
					...
					else if(ori_f==210) //ori_2
					{
						...
						command_cartesian_position.pose.position.x = (robot_x - ((y+80)/1000)*tan(30.0*PI/180.0) );
						command_cartesian_position.pose.position.y = (y+80)/1000;
						command_cartesian_position.pose.position.z = (z + 130)/1000;
					}

					else if(ori_f==195) //ori_3
					{
						//ROS_INFO("ori_3");
						...
						command_cartesian_position.pose.position.x = (robot_x - ((y+80)/1000)*tan(15.0*PI/180.0) );
						command_cartesian_position.pose.position.y = (y+80)/1000;
						command_cartesian_position.pose.position.z = (z + 130)/1000;
					}
					...
					else if(ori_f==180) //ori_4
					{
						...
						command_cartesian_position.pose.position.x = robot_x;
						command_cartesian_position.pose.position.y = y/1000;
						command_cartesian_position.pose.position.z = (z + 130)/1000;
					}

					else if(ori_f==165) //ori_5
					{
						//ROS_INFO("ori_5");
					...
					if (data_not_same)
					{
						...
						if(x!=0 && y!=0 && z!=0)
						{
							...
							else if(control_mode==1)//impedance control
							{
								//ROS_INFO("impedance control");
