...
int main (int argc, char **argv)
{
	...
	while (ros::ok() && isRobotConnected && use_cartesian_command)
	{
		...
		{
			ROS_INFO("set pose --- ori: %f, pos_x: %f, pos_y: %f, pos_z:%f", ori, pos_x, pos_y, pos_z);
			setRobotPose(ori, pos_x, pos_y, pos_z, 0); ///set roughness...
		}
...
void setRobotPose(int ori, float x, float y, float z, int r)
{
	...
	command_cartesian_position.pose.orientation.x = 0; //TODO: relative angle!!! use r
	command_cartesian_position.pose.orientation.y = 1.5708;
	command_cartesian_position.pose.orientation.z = (ori*PI)/180;

	command_cartesian_position.pose.orientation.w = 0; //TODO: relative velocity!!! use r
}
